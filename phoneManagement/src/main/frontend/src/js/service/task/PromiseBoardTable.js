...
export function PromiseBoardTable({items, onChangeState}){
            ...
            <div className={Board.promise}>
                <form>
                    <ul className={Board.promise_list}>
                        {
                            items && items.map((v,i)=>{
                                return <PromiseItem key={i} item={v} onUpdate={onChangeState}/>
                            })
                        }
                    </ul>
                </form>
...
function PromiseItem({key, item, onUpdate}){
    return (
        <li key={key} className={Board.promise_item}>
                        ...
                        <ul className="option_list">
                            {
                                item.pm_list && item.pm_list.map((v,i)=>{
                                    return <PromiseOptionItem key={i} disabled={v.checked}
                                                              onClick={()=>{
